...
class ConfigurationReadinessManager:
    ...
    def _add_phase_section(report):
        ...
        if expected_groups:
            expected_group_set = {code.upper() for code in expected_groups}
            groups = [
                group for group in groups
                if (group.get("phase_group_code") or "").upper()
                in expected_group_set
            ]
            options = [
                option for option in options
                if (option.get("phase_group_code") or "").upper()
                in expected_group_set
            ]
