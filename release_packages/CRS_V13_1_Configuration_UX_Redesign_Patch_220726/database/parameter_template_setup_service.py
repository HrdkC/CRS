...
class ParameterTemplateSetupService:
    ...
    @staticmethod
    def preview_from_configured_array(
        *, machine_id, stage_id, source_tag_id, start_index, end_index,
        name_prefix, unit, min_value, max_value, default_value,
    ) -> dict[str, Any]:
        """Validate and preview a guided template without changing the database."""
        source_id = _as_int(source_tag_id, "Configured PLC array")
        ...
        if end_value < start_value:
            raise ParameterTemplateSetupError(
                "Last used index cannot be lower than first used index."
            )
        if end_value - start_value + 1 > 1000:
            raise ParameterTemplateSetupError(
        ...
        if default_number < min_number or default_number > max_number:
            raise ParameterTemplateSetupError(
                "Default value must be within the minimum and maximum limits."
            )
        conn = get_connection()
        try:
            tag_row = conn.execute(
                ...
                """,
                (source_id, int(machine_id), int(stage_id)),
            ).fetchone()
            if not tag_row:
                raise ParameterTemplateSetupError(
                    "The selected PLC array is not configured for this machine/stage."
                )
            tag = dict(tag_row)
            tag_type = str(tag.get("tag_type") or "").upper()
            if int(tag.get("is_array") or 0) != 1 or tag_type not in _NUMERIC_ARRAY_TYPES:
                raise ParameterTemplateSetupError("Select a configured numeric PLC array.")
            configured_start, configured_end = _source_tag_bounds(tag)
            if start_value < configured_start or end_value > configured_end:
                raise ParameterTemplateSetupError(
                    f"Used indexes must remain inside {configured_start} to {configured_end}."
                )
            existing = {
                int(row[0])
                for row in conn.execute(
                    """
                    SELECT COALESCE(plc_array_index, tag_index)
        ...
        for array_index in range(start_value, end_value + 1):
            rows.append({
                "index": array_index,
                "parameter_name": f"{prefix} {array_index:03d}",
                "unit": unit_text,
                "min_value": min_number,
                "max_value": max_number,
                "default_value": default_number,
                "action": "KEEP" if array_index in existing else "CREATE",
            })
        return {
            "source_tag": tag,
            ...
            "keep_count": sum(row["action"] == "KEEP" for row in rows),
            "rows": rows,
            "form": {
                "source_tag_id": source_id, "start_index": start_value,
                "end_index": end_value, "name_prefix": prefix, "unit": unit_text,
    ...
    @staticmethod
    def get_compatible_template_sources(machine_id, stage_id):
        conn = get_connection()
        try:
            rows = conn.execute(
                """
                SELECT
                    ...
                    SUM(CASE WHEN COALESCE(p.used, 1) = 1 THEN 1 ELSE 0 END) AS used_count
                FROM machine_stages target
                INNER JOIN machine_stages s
                    ON UPPER(s.stage_type) = UPPER(target.stage_type)
                   AND s.id <> target.id
                INNER JOIN tbm_machines m ON m.id = s.machine_id
                INNER JOIN parameter_definitions p ON p.stage_id = s.id
                WHERE target.id = ? AND target.machine_id = ?
                GROUP BY s.id, s.machine_id, m.machine_code, s.stage_type
                HAVING COUNT(p.id) > 0
    ...
    @staticmethod
    def preview_copy(machine_id, stage_id, source_stage_id):
        source_id = _as_int(source_stage_id, "Source template")
        sources = ParameterTemplateSetupService.get_compatible_template_sources(
            machine_id, stage_id
        )
        source = next((row for row in sources if int(row["stage_id"]) == source_id), None)
        if not source:
            raise ParameterTemplateSetupError(
                "Select a template from a compatible stage type."
            )
        conn = get_connection()
        try:
            existing = {
            ...
            rows = [
                ...
                for row in conn.execute(
                    ...
                    WHERE stage_id = ?
                    ORDER BY COALESCE(plc_array_index, tag_index), tag_index
                    """,
                    (source_id,),
                ).fetchall()
            ]
            ...
            conn.close()
        for row in rows:
            index = row.get("plc_array_index")
            if index is None:
                index = row.get("tag_index")
            row["action"] = "KEEP" if int(index) in existing else "COPY"
        return {
            "source": source,
            "rows": rows,
            "copy_count": sum(row["action"] == "COPY" for row in rows),
            "keep_count": sum(row["action"] == "KEEP" for row in rows),
        }

    @staticmethod
    def copy_missing_from_stage(
        *, machine_id, stage_id, source_stage_id, username, role, reason,
        request_metadata=None,
    ):
        reason_text = _validate_reason(reason)
        preview = ParameterTemplateSetupService.preview_copy(
            machine_id, stage_id, source_stage_id
        )
        ...
        with transaction(immediate=True) as conn:
            ...
            for row in preview["rows"]:
                ...
                cursor = conn.execute(
                    ...
                    (machine_id, stage_id, tag_index, plc_array_index,
                     parameter_name, parameter_class, unit, min_value, max_value,
                     default_value, datatype, english_memo, used, created_by)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        int(machine_id), int(stage_id), row.get("tag_index"),
                        row.get("plc_array_index"), row.get("parameter_name"),
                        row.get("parameter_class"), row.get("unit"),
                        row.get("min_value"), row.get("max_value"),
                        row.get("default_value"), row.get("datatype"),
                        row.get("english_memo"), row.get("used", 1), username,
                    ),
                )
            ...
            for parameter_id in created_ids:
                default_row = conn.execute(
                    "SELECT default_value FROM parameter_definitions WHERE id = ?",
                    (parameter_id,),
                ).fetchone()
                cursor = conn.execute(
                    """
                    ...
                    SELECT id, ?, ?, 0 FROM recipes
                    WHERE machine_id = ? AND stage_id = ?
                    """,
                    (
                        parameter_id, default_row["default_value"],
                        int(machine_id), int(stage_id),
                    ),
                )
                backfilled += max(0, int(cursor.rowcount or 0))
            AuditManager.log_event(
                username=username, role=role,
                action="PARAMETER_TEMPLATE_COPIED",
                change_source="PARAMETER_TEMPLATE_WIZARD",
                record_id=int(stage_id),
                old_value=f"source_stage_id={int(source_stage_id)}",
                new_value=f"created={len(created_ids)}; backfilled={backfilled}",
                reason=reason_text,
        ...
        return {"created_count": len(created_ids), "backfilled_count": backfilled}
