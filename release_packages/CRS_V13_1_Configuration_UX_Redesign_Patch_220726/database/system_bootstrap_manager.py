...
class CRSSystemBootstrapManager:
    ...
    def _steps(self):
        ...
        BootstrapStep("08 Audit/Security", "Configuration workflow schema", lambda: self._call("database.configuration_workflow_schema", "apply_configuration_workflow_schema")),
    ...
    def run(self):
        ...
        self._print("Site Migration: EXCLUDED (generic recovery only)")
        self._print(f"Strict Mode  : {'YES' if self.strict else 'NO'}")
        self._print("")
...
def build_argument_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Create or upgrade the generic CRS workstation database and "
            "verify required tables."
        ),
        epilog=(
            "Machine-specific recipe and phase data migrations are always "
            "excluded from workstation recovery."
        ),
    )
    ...
    seed_group.add_argument(
        "--seed-users",
        action="store_true",
        help="create configured recovery users when missing",
    )
    seed_group.add_argument(
        "--no-seed-users",
        action="store_true",
        help="do not create recovery users",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
        help="return failure for any warning or optional step failure",
    )
    return parser
...
def main(argv=None):
    ...
    if result["status"] == "SUCCESS":
        return 0
    if result["status"] == "WARNING" and not args.strict:
        return 2
    return 1
