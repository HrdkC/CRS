...
def test_guided_setup_preview_is_read_only_and_identifies_existing_rows():
    ...
    assert preview["create_count"] == 3
    assert preview["keep_count"] == 0
    conn = get_connection()
    try:
        assert conn.execute("SELECT COUNT(*) FROM parameter_definitions").fetchone()[0] == 0
